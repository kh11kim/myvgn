...
def run(
    ...
    """
    sim = GraspSim(scene, object_set, gui=sim_gui, seed=seed)
    ...
    for _ in tqdm.tqdm(range(num_rounds)):
        ...
        while consecutive_failures < MAX_CONSECUTIVE_FAILURES: #sim.num_objects > 0 and 
            timings = {}

            # scan the scene
            depth_imgs, extrinsics = sim.render_images(n=12)
