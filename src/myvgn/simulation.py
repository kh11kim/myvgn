...
class GraspSim(object):
    ...
    def generate_air_scene(self):
        # set objects
        urdf = self.rng.choice(self.object_urdfs, size=1)
        rotation = Rotation.random(random_state=self.rng)
        xyz = self.rng.uniform(1.0 / 3.0 * self.size, 2.0 / 3.0 * self.size, 3) #np.array([self.size/2, self.size/2, self.size/2]) #
        pose = Transform(rotation, np.r_[xyz])
        scale = self.rng.uniform(0.8, 1.0)
        self.world.load_urdf(urdf[0], pose, scale=self.global_scaling * scale)
